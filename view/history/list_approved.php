...
$q = "SELECT * FROM update_boq WHERE status3  = 'OK' and status2  = 'OK' and status1 = 'OK' ORDER BY dt DESC";
$r = $conn->query($q);
        ...
        if ($r = $conn->query($q)) {
            if ($r->num_rows!==0) {
                ...
                while ($update_boq=$r->fetch_assoc()) {
                    ...
                    <td><a href='./view/printba.php?id=$id' class='btn btn-success btn-sm'>Berita Acara</td>
                    <td>$print1</td>
                    <td>$print2</td>
                    <td>$print3</td>
                    </tr>";
