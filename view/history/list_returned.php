...
$q = "SELECT * FROM update_boq WHERE status3  = 'Returned' or status2  = 'Returned' or status1 = 'Returned'  ORDER BY dt DESC";
$r = $conn->query($q);
        ...
        if ($r = $conn->query($q)) {
            if ($r->num_rows!==0) {
                ...
                while ($update_boq=$r->fetch_assoc()) {
                    ...
                    <td><button  class='btn btn-warning btn-sm' data-toggle='modal' data-target='#modalEditorder$id'><i class='glyphicon glyphicon-check'> Edit</i>
                    </button></td>
                    <td><button  class='btn btn-success btn-sm' data-toggle='modal' data-target='#modalreorder$id'><i class='glyphicon glyphicon-check'> Re-Order</i>
                    </button></td>
